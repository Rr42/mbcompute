...
# Description:
#  Eighth self test (part 1) for console application.
#  This test checks some specific test involving computation
#  using SI prefixes.
#############################################################################
...
printf "Running test: 15750/1k\n"
result=`$mb_app $options --command="15750/1k\nexit" | tail -n 1`
printf "Result: $result"
if [ "$result" == "15.75" ]; then
    printf " - PASS\n"
else
    printf " - FAIL\n"
fi
